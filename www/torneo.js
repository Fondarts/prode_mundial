...
async function renderizarTorneo() {
    ...
    if (misTorneos.length === 0) {
        ...
        misTorneos.forEach((torneoData, index) => {
            ...
            const btnCompartir = torneoItem.querySelector('.btn-compartir-torneo');
            if (btnCompartir) {
...
// ============================================
// FUNCIONES PARA COMPARTIR TORNEO POR LINK
// ============================================

// Detectar código de torneo en la URL al cargar la app
async function detectarTorneoEnURL() {
    const params = new URLSearchParams(window.location.search);
    const codigoTorneo = params.get('torneo');
    
    if (codigoTorneo) {
        // Esperar un poco para que se inicialice todo
        await new Promise(resolve => setTimeout(resolve, 500));
        
        // Mostrar landing del torneo
        await mostrarLandingTorneo(codigoTorneo);
    }
}

// Mostrar landing del torneo cuando se accede desde un link
async function mostrarLandingTorneo(codigo) {
    // Cargar datos desde Supabase si es necesario
    if (usarSupabase() && typeof cargarDatosDesdeSupabase === 'function') {
        try {
            const datosSupabase = await cargarDatosDesdeSupabase();
            if (datosSupabase) {
    ...
    // Verificar si el torneo existe
    let torneo = torneos[codigo];
    
    // Si no existe localmente, intentar obtenerlo de Supabase
    if (!torneo && usarSupabase() && typeof obtenerTorneoPorCodigoSupabase === 'function') {
        const torneoSupabase = await obtenerTorneoPorCodigoSupabase(codigo);
        if (torneoSupabase) {
            const participantesSupabase = await obtenerParticipantesSupabase(codigo);
            torneo = {
                ...
                fechaCreacion: torneoSupabase.fechaCreacion,
                resultadosReales: torneoSupabase.resultadosReales || {},
                esPrivado: torneoSupabase.es_privado || false,
                clave: torneoSupabase.clave || null
            };
            // Guardar en memoria local
            torneos[codigo] = torneo;
        }
    }
    ...
    if (!torneo) {
        await mostrarModal({
            titulo: typeof t === 'function' ? t('error') : 'Error',
            mensaje: typeof t === 'function' ? t('torneoNoEncontrado') : 'Torneo no encontrado. El código del torneo no es válido.',
            cancelar: false
        });
        // Limpiar URL
        limpiarURLTorneo();
    ...
    const yaEstaEnTorneo = torneo.participantes && torneo.participantes.some(p => p && p.nombre === nombreUsuario);
    ...
    // Crear modal de landing
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    overlay.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); z-index: 10000; display: flex; align-items: center; justify-content: center;';
    
    const modal = document.createElement('div');
    modal.className = 'modal-torneo-landing';
    modal.style.cssText = 'background: white; border-radius: 12px; padding: 0; max-width: 500px; width: 95%; max-height: 90vh; overflow-y: auto; box-shadow: 0 4px 20px rgba(0,0,0,0.3);';
    
    let contenidoBoton = '';
    if (yaEstaEnTorneo) {
        contenidoBoton = `
            <button id="btn-ver-torneo" style="width: 100%; padding: 14px; background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); color: white; border: none; border-radius: 8px; font-size: 1.1em; font-weight: 600; cursor: pointer; margin-top: 20px;">
                ${typeof t === 'function' ? t('verTorneo') : '🏆 Ver Torneo'}
            </button>
        `;
    } else if (nombreUsuario) {
        contenidoBoton = `
            <button id="btn-unirse-torneo" style="width: 100%; padding: 14px; background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); color: white; border: none; border-radius: 8px; font-size: 1.1em; font-weight: 600; cursor: pointer; margin-top: 20px;">
                ${typeof t === 'function' ? t('unirseAlTorneo') : '✅ Unirse al Torneo'}
            </button>
        `;
    } else {
        contenidoBoton = `
            <button id="btn-login-unirse" style="width: 100%; padding: 14px; background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); color: white; border: none; border-radius: 8px; font-size: 1.1em; font-weight: 600; cursor: pointer; margin-top: 20px;">
                ${typeof t === 'function' ? t('iniciarSesionParaUnirse') : '🔐 Iniciar Sesión para Unirse'}
            </button>
        `;
    }
    
    modal.innerHTML = `
        <div style="background: linear-gradient(135deg, #1e3a8a 0%, #2a5298 100%); color: white; padding: 24px; border-radius: 12px 12px 0 0;">
            <h2 style="margin: 0 0 8px 0; font-size: 1.8em;">🏆 ${torneo.nombre || `Torneo ${codigo}`}</h2>
            <p style="margin: 0; opacity: 0.9; font-size: 0.95em;">${typeof t === 'function' ? t('creadoPor') : 'Creado por'}: <strong>${torneo.creadoPor || (typeof t === 'function' ? t('desconocido') : 'Desconocido')}</strong></p>
        </div>
        <div style="padding: 24px;">
            <div style="margin-bottom: 20px;">
                <p style="margin: 8px 0; color: #475569; font-size: 1em;">
                    <strong>👥 ${typeof t === 'function' ? t('participantes') : 'Participantes'}:</strong> ${torneo.participantes ? torneo.participantes.length : 0}
                </p>
                ${esPrivadoFinal ? `<p style="margin: 8px 0; color: #475569; font-size: 1em;"><strong>🔒 ${typeof t === 'function' ? t('torneoPrivado') : 'Torneo Privado'}</strong></p>` : ''}
                ${yaEstaEnTorneo ? `<p style="margin: 8px 0; color: #10b981; font-size: 1em; font-weight: 600;">✅ ${typeof t === 'function' ? t('yaEstasEnEsteTorneo') : 'Ya estás en este torneo'}</p>` : ''}
            </div>
            <p style="color: #666; font-size: 0.95em; line-height: 1.6; margin-bottom: 20px;">
                ${typeof t === 'function' ? t('descripcionLandingTorneo') : 'Predice los resultados del Mundial 2026 y compite con tus amigos. ¡Únete ahora!'}
            </p>
            ${contenidoBoton}
            <button id="btn-cerrar-landing" style="width: 100%; padding: 10px; background: #e5e7eb; color: #475569; border: none; border-radius: 8px; font-size: 0.95em; font-weight: 600; cursor: pointer; margin-top: 12px;">
                ${typeof t === 'function' ? t('cerrar') : 'Cerrar'}
            </button>
        </div>
    `;
    ...
    // Botón de login para unirse
    const btnLoginUnirse = document.getElementById('btn-login-unirse');
    if (btnLoginUnirse) {
        btnLoginUnirse.addEventListener('click', async () => {
            ...
            if (typeof mostrarDialogoLogin === 'function') {
                ...
                if (loginExitoso) {
                    // Después del login, volver a mostrar el landing
                    await mostrarLandingTorneo(codigo);
                } else {
                    // Si canceló el login, limpiar URL
                    limpiarURLTorneo();
                }
            }
        });
    ...
    // Botón de ver torneo (si ya está en el torneo)
...
async function compartirTorneo(codigo) {
    const torneo = torneos[codigo];
    ...
    // Obtener traducciones de forma robusta
    let textoUnete = 'Únete a mi torneo';
    let textoPredice = 'Predice los resultados del Mundial 2026 y compite conmigo!';
    
    // Intentar obtener traducciones del objeto translations directamente
    try {
        if (typeof window !== 'undefined' && window.translations) {
            ...
            if (traducciones) {
                if (traducciones.uneteAMiTorneo) textoUnete = traducciones.uneteAMiTorneo;
                if (traducciones.prediceResultadosMundial) textoPredice = traducciones.prediceResultadosMundial;
            }
        } else if (typeof window !== 'undefined' && typeof window.t === 'function') {
            const uneteTraducido = window.t('uneteAMiTorneo');
            const prediceTraducido = window.t('prediceResultadosMundial');
            if (uneteTraducido && uneteTraducido !== 'uneteAMiTorneo') textoUnete = uneteTraducido;
            if (prediceTraducido && prediceTraducido !== 'prediceResultadosMundial') textoPredice = prediceTraducido;
        } else if (typeof t === 'function') {
            const uneteTraducido = t('uneteAMiTorneo');
            const prediceTraducido = t('prediceResultadosMundial');
            if (uneteTraducido && uneteTraducido !== 'uneteAMiTorneo') textoUnete = uneteTraducido;
            if (prediceTraducido && prediceTraducido !== 'prediceResultadosMundial') textoPredice = prediceTraducido;
        }
    } catch (e) {
...
// Limpiar parámetro de torneo de la URL
function limpiarURLTorneo() {
